...
describe CatsDealers do
  let(:cats_dealers) { described_class.new }
  let(:all_offers) do
    [
  ...
  describe '#find_offers' do
    context 'when search params present' do
      ...
      it 'filters offeres by type and location' do
        VCR.use_cassette('cats_dealers') do
          expect(cats_dealers.find_offers(search_params)).to eq(expected_results)
        end
      end
      ...
      context 'when there are no offers matching the search params' do
        ...
        it 'returns empty array' do
          VCR.use_cassette('cats_dealers') do
            expect(cats_dealers.find_offers(search_params)).to eq([])
          end
        end
      end

      context 'when one search param is specified' do
    ...
    context 'when search params are missing' do
      ...
      it 'returns all offers' do
        VCR.use_cassette('cats_dealers') do
          expect(cats_dealers.find_offers(search_params)).to eq(all_offers)
        end
      end
    end
  end
  ...
  describe '#all_offers' do
    it 'gets offers from all dealers' do
      VCR.use_cassette('cats_dealers') do
        expect(cats_dealers.all_offers).to eq(all_offers)
      end
    end
  end
end
